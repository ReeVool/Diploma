...
public class QuerriesSearch
{
    [Obsolete]
    public IQueryable<QuerriesToBuy> Search(string searchTerm, DbContext db)
    {
        var query = db.Set<QuerriesToBuy>().AsQueryable();

        if (string.IsNullOrWhiteSpace(searchTerm))
            return query;

        var predicate = PredicateBuilder.False<QuerriesToBuy>();
        var properties = typeof(QuerriesToBuy).GetProperties();

        foreach (var prop in properties)
        {
            var param = Expression.Parameter(typeof(QuerriesToBuy), "x");
            ...
            if (prop.PropertyType == typeof(string))
            {
                ...
                predicate = predicate.Or(Expression.Lambda<Func<QuerriesToBuy, bool>>(body, param));
            }
            // Для числовых полей
            else if (IsNumericType(prop.PropertyType))
            {
                if (TryConvertToType(searchTerm, prop.PropertyType, out object convertedValue))
                {
                    ...
                    predicate = predicate.Or(Expression.Lambda<Func<QuerriesToBuy, bool>>(equalsExpr, param));
                }
            }
        }
