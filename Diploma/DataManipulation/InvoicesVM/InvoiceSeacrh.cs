...
{
    [Obsolete]
    public IQueryable<Invoices> Search(string searchTerm, DbContext db)
    {
        var query = db.Set<Invoices>().AsQueryable();

        if (string.IsNullOrWhiteSpace(searchTerm))
            return query;

        var predicate = PredicateBuilder.False<Invoices>();
        var properties = typeof(Invoices).GetProperties();

        foreach (var prop in properties)
        {
            var param = Expression.Parameter(typeof(Invoices), "x");
            ...
            if (prop.PropertyType == typeof(string))
            {
                ...
                predicate = predicate.Or(Expression.Lambda<Func<Invoices, bool>>(body, param));
            }
            // Для числовых полей
            else if (IsNumericType(prop.PropertyType))
            {
                if (TryConvertToType(searchTerm, prop.PropertyType, out object convertedValue))
                {
                    ...
                    predicate = predicate.Or(Expression.Lambda<Func<Invoices, bool>>(equalsExpr, param));
                }
            }
            // Для полей даты
            ...
            {
                if (DateOnly.TryParse(searchTerm, out DateOnly dateValue))
                {
                    var equalsExpr = Expression.Equal(
                        propExpr,
                        Expression.Constant(dateValue, prop.PropertyType)
                    );
                    predicate = predicate.Or(Expression.Lambda<Func<Invoices, bool>>(equalsExpr, param));
                }
            }
