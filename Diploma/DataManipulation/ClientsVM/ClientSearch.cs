...
{
    [Obsolete]
    public IQueryable<Clients> Search(string searchTerm, DbContext db)
    {
        var query = db.Set<Clients>().AsQueryable();

        if (string.IsNullOrWhiteSpace(searchTerm))
            return query;

        var predicate = PredicateBuilder.False<Clients>();
        var properties = typeof(Clients).GetProperties();

        foreach (var prop in properties)
        {
            var param = Expression.Parameter(typeof(Clients), "x");
            ...
            if (prop.PropertyType == typeof(string))
            {
                ...
                predicate = predicate.Or(Expression.Lambda<Func<Clients, bool>>(body, param));
            }
            // Для числовых полей
            else if (IsNumericType(prop.PropertyType))
            {
                if (TryConvertToType(searchTerm, prop.PropertyType, out object convertedValue))
                {
                    ...
                    predicate = predicate.Or(Expression.Lambda<Func<Clients, bool>>(equalsExpr, param));
                }
            }
        }
