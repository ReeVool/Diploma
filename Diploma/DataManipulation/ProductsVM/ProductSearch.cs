...
{
    [Obsolete]
    public IQueryable<Products> Search(string searchTerm, DbContext db)
    {
        var query = db.Set<Products>().AsQueryable();
        ...
        foreach (var prop in properties)
        {
            ...
            if (prop.PropertyType == typeof(string))
            {
                ...
                predicate = predicate.Or(Expression.Lambda<Func<Products, bool>>(body, param));
            }
            // Для числовых полей
            else if (IsNumericType(prop.PropertyType))
            {
                if (TryConvertToType(searchTerm, prop.PropertyType, out object convertedValue))
                {
                    ...
                    predicate = predicate.Or(Expression.Lambda<Func<Products, bool>>(equalsExpr, param));
                }
            }
        }
