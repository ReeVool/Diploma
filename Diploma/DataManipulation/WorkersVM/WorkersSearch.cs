...
public class WorkersSearch
{
    [Obsolete]
    public IQueryable<Workers> Search(string searchTerm, DbContext db)
    {
        var query = db.Set<Workers>().AsQueryable();

        if (string.IsNullOrWhiteSpace(searchTerm))
            return query;

        var predicate = PredicateBuilder.False<Workers>();
        var properties = typeof(Workers).GetProperties();

        foreach (var prop in properties)
        {
            var param = Expression.Parameter(typeof(Workers), "x");
            ...
            if (prop.PropertyType == typeof(string))
            {
                ...
                predicate = predicate.Or(Expression.Lambda<Func<Workers, bool>>(body, param));
            }
            // Для числовых полей
            else if (IsNumericType(prop.PropertyType))
            {
                if (TryConvertToType(searchTerm, prop.PropertyType, out object convertedValue))
                {
                    ...
                    predicate = predicate.Or(Expression.Lambda<Func<Workers, bool>>(equalsExpr, param));
                }
            }
            // Для полей даты
