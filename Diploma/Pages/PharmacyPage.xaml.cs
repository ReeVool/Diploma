...
using Diploma.ViewModels.PharmacyVM;
...
namespace Diploma.Views
{
    public partial class PharmacyPage : Page
    {
        ...
        void LoadData()
        {
            ...
            {
                DataTable.ItemsSource = context.Pharmacy.ToList();
            }
        ...
        {
            var selItem = DataTable.SelectedItem as Pharmacy;

            if (selItem == null)
                return;

            PharmacyEditWindow window = new(selItem.Id);
            window.ShowDialog();
            LoadData();
        }
        ...
        void DeleteInformation()
        {
            ...
            if (res == MessageBoxResult.Yes)
            {
                try
                {
                    using (var context = new AppDbContext())
                    {
                        context.Pharmacy.Remove(selItem);
                        context.SaveChanges();
                    }
                    LoadData();
                }
